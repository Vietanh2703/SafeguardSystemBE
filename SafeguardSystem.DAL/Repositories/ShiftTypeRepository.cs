...
namespace SafeguardSystem.DAL.Repositories
{
    public class ShiftTypeRepository : GenericRepository<ShiftType>, IShiftTypeRepository
    {
        ...
        public bool ValidateShiftTime(TimeSpan startTime, TimeSpan endTime, out string errorMessage)
        {
            ...
            {
                TimeSpan durationOvernight = (TimeSpan.FromHours(24) - startTime) + endTime;
        ...
        public bool IsValidTimeFormat(TimeSpan? time)
        {
            ...
            try
            {
                string timeString = time.Value.ToString("HH:mm:ss", CultureInfo.InvariantCulture);
        ...
        {
            double s1 = ConvertTimeToDouble(start1);
            double e1 = ConvertTimeToDouble(end1);
            double s2 = ConvertTimeToDouble(start2);
            double e2 = ConvertTimeToDouble(end2);
