...
public class TeamService : ITeamService
{
    ...
    public async Task<ResponseDTO> GetAllTeamsAsync(int pageNumber, int pageSize)
    {
        ...
        if (paginatedTeams == null || !paginatedTeams.Any()) return new ResponseDTO("Empty team in list.", 200);
        var teamDTOs = paginatedTeams.Where(t => !t.IsDeleted)
            .Select(t => new TeamDTO
            {
                Name = t.Name
            }).ToList();
        return new ResponseDTO("Retrieve team: ", 200, true,
            new PaginatedList<TeamDTO>(teamDTOs, paginatedTeams.Count, pageSize, pageNumber));
    }
    ...
    public async Task<ResponseDTO> GetTeamByNameAsync(string teamName)
    {
        ...
        if (results == null) return new ResponseDTO("Team not found", 404);
    ...
    public async Task<ResponseDTO> AssignGuardToTeamAsync(Guid teamId, Guid guardId)
    {
        ...
        if (guard == null) return new ResponseDTO("Guard not found", 404);
    ...
    public async Task<ResponseDTO> RemoveGuardFromTeamAsync(Guid teamGuardId)
    {
        ...
        if (teamGuard == null) return new ResponseDTO("Guard not found in team", 404);
    ...
    {
        if (teamDTO == null) return new ResponseDTO("Invalid team info", 400);
        var team = new Team
        {
            Name = teamDTO.Name
        };
        var result = await _unitOfWork.Teams.CreateTeamAsync(team);
        if (result == null) return new ResponseDTO("Create team failed", 400);
    ...
    public async Task<ResponseDTO> UpdateTeamAsync(Guid teamId, TeamDTO teamDTO)
    {
        ...
        if (result == null) return new ResponseDTO("Team not found", 404);
        result.Name = teamDTO.Name;
        await _unitOfWork.SaveChangeAsync();
        if (result == null) return new ResponseDTO("Update team failed", 400);
    ...
    public async Task<ResponseDTO> DeleteTeamAsync(Guid teamId)
    {
        ...
        if (!teamExists) return new ResponseDTO("Team not found", 404);

        // Kiểm tra team có bảo vệ không
        var teamHasGuards = await _unitOfWork.TeamGuards.Any(tg => tg.TeamId == teamId);
        if (teamHasGuards) return new ResponseDTO("Cannot delete team with existing guards", 400);
