...
namespace SafeguardSystem.BLL.Services
{
    public class LoginRequestService : ILoginRequestService
    {
        ...
        public async Task<ResponseDTO> GetAllRequests(int pageNumber, int pageSize)
        {
            ...
            {
                return new ResponseDTO("Invalid page number or page size", 400, false);
            }
            ...
            {
                return new ResponseDTO("No requests found in this list.", 200, false);
            }
            ...
            return new ResponseDTO("Login requests retrieved successfully", 200, true, new PaginatedList<LoginRequestDTO>(requestDTOs, paginatedRequests.Count, pageSize, pageNumber));
        }
        ...
        public async Task<ResponseDTO> ApprovalLoginGoogle(Guid requestId, string status, string reason)
        {
            ...
            {
                return new ResponseDTO(AdminApprovalMessage.Missing, 404, false);
            }

            // Check if the login request has been approved before
            if (loginRequest.Status != "PENDING")
            {
                return new ResponseDTO(AdminApprovalMessage.Same, 400, false);
            }
