...
public class AuthService : IAuthService
{
    ...
    public async Task<ResponseDTO> LoginAsync(LoginDTO loginDTO)
    {
        ...
        if (string.IsNullOrEmpty(firebaseApiKey) || string.IsNullOrEmpty(firebaseUrl))
            return new ResponseDTO("Cannot connect to Firebase Service", 400);
        ...
        {
            return new ResponseDTO($"Firebase login request failed: {ex.Message}", 500);
        }

        if (!response.IsSuccessStatusCode) return new ResponseDTO("Invalid email or password", 400);
        ...
        if (!root.TryGetProperty("localId", out var userIdElement) || string.IsNullOrEmpty(userIdElement.GetString()))
            return new ResponseDTO("Cannot collect any data from Firebase", 400);

        var firebaseUid = userIdElement.GetString() ?? string.Empty;
        var email = root.GetProperty("email").GetString() ?? string.Empty;
        var displayName = root.GetProperty("displayName").GetString() ?? email;
        ...
        if (user.IsDeleted) return new ResponseDTO("This account does not exist.", 400);
        if (!user.IsActive && user.ActivationTokenExpiry == null)
            return new ResponseDTO("Your account is not verified, please check your email.", 400);

        if (user.ActivationTokenExpiry != null) return new ResponseDTO("This account does not verify OTP.", 400);


        if (user.IsLocked) return new ResponseDTO("Your account has been banned. Please contact your Admin", 400);
        ...
        {
            new(JwtConstant.KeyClaim.Email, user.Email ?? string.Empty),
            new(JwtConstant.KeyClaim.userId, user.UserId),
            new(JwtConstant.KeyClaim.fullName, user.FullName ?? string.Empty),
            new(JwtConstant.KeyClaim.Role, roleName)
        };
        ...
        {
            return new ResponseDTO($"Error saving refresh token: {ex.Message}", 500);
        }
    ...
    public async Task<ResponseDTO> SignInWithGoogleAsync(GoogleLoginDTO googleLoginDTO)
    {
        ...
        {
            return new ResponseDTO($"Error verifying Google token: {ex.Message}", 400);
        }
        ...
        if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(email))
            return new ResponseDTO("Invalid Google token", 400);
        ...
        if (user.IsDeleted) return new ResponseDTO("Your account does not exist.", 400);
    ...
    public async Task<ResponseDTO> RefreshBothTokens(string oldAccessToken, string refreshTokenKey)
    {
        ...
        if (claimsPrincipal == null) return new ResponseDTO("Invalid refresh token", 400);
        ...
        if (refreshTokenDTO == null || refreshTokenDTO.IsRevoked)
            return new ResponseDTO("Refresh token not found or has been revoked", 403);
        ...
        if (tokenExpirationDate == null || DateTime.UtcNow > tokenExpirationDate)
            return new ResponseDTO("Refresh token expired, please login again", 403);
        ...
        {
            return new ResponseDTO($"Error refreshing tokens: {ex.Message}", 500);
        }
    ...
    public async Task<ResponseDTO> LogoutAsync(string refreshTokenKey)
    {
        ...
        if (refreshToken == null) return new ResponseDTO("Refresh token not found", 404);
        ...
        {
            return new ResponseDTO($"Error during logout: {ex.Message}", 500);
        }
