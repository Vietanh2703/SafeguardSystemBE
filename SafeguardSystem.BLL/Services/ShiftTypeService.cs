...
namespace SafeguardSystem.BLL.Services
{
    public class ShiftTypeService : IShiftTypeService
    {
        ...
        public async Task<ResponseDTO> GetAllShiftTypesAsync()
        {
            ...
            {
                return new ResponseDTO("No shift type found", 200, true, null);
            }
        ...
        public async Task<ResponseDTO> CreateShiftTypeAsync(ShiftTypeDTO shiftTypeDTO)
        {
            try
            {
                ...
                {
                    return new ResponseDTO("Invalid shift time.", 400, false);
                }
                ...
                if (existingShifts.Any() && await _unitOfWork.ShiftTypes.IsTimeConflictAsync(shiftTypeDTO.StartTime, shiftTypeDTO.EndTime))
                {
                    return new ResponseDTO("Shift time conflicts with an existing shift.", 400, false);
                }

                if (!_unitOfWork.ShiftTypes.ValidateShiftTime(shiftTypeDTO.StartTime, shiftTypeDTO.EndTime, out string errorMessage))
                {
                    return new ResponseDTO(errorMessage, 400, false);
                }
            ...
            {
                return new ResponseDTO($"Error creating shift type: {ex.Message}", 500, false);
            }
        }
        ...
        public async Task<ResponseDTO> UpdateShiftTypeAsync(Guid typeId, ShiftTypeDTO shiftTypeDTO)
        {
            try
            {
                ...
                if (existingShift == null || existingShift.IsDeleted)
                {
            ...
            {
                return new ResponseDTO($"Error updating shift type: {ex.Message}", 500, false);
            }
        }
        ...
        public async Task<ResponseDTO> DeleteShiftTypeAsync(Guid typeId)
        {
            ...
            {
                return new ResponseDTO($"Error deleting shift type: {ex.Message}", 500, false);
            }
        }
